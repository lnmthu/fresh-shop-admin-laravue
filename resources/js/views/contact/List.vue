<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="Unique_ID" width="100">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="Message" width="300">
        <template slot-scope="scope">
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            size="small"
            icon="el-icon-delete"
            @click="handleRestatus(scope.row)"
          >Restatus</el-button>
...
export default {
  ...
  methods: {
    ...
    async getListPaginated() {
      this.loading = true;
      const { data, meta } = await contactResource.list(this.listQuery);
    ...
    handleDelete(unique_id, name) {
      this.$confirm(
        'This will momentarily delete contact ' + name + '. Continue?',
        'Warning',
        {
    ...
    // Restatus
    handleRestatus(data) {
      var postForm = { status: 0 };
      if (data.status === 0) {
        postForm.status = 1;
      }
      contactResource
        .update(data.unique_id, postForm)
        .then((response) => {
          this.$message({
            type: 'success',
            message: 'Contact of ' + data.name + ' is ' + this.getStatus(postForm.status),
          });
          this.getListPaginated();
        })
        .catch((error) => {
